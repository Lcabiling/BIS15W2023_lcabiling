...
filter(fish, lakeid != "AL")
```
an object will appear on the environment panel, it can be a data frame, a data matrix, a name that you put a value to, etc. 
...
```
length is a numeric, %in% is "look within", concatinate (value, value) 
...
You can also extract observations "near" a certain value but you need to specify a tolerance (tol). 
```{r}
...
Notice that the `|` = "or"  operator generates a different result.
either values are met
```{r}
filter(fish, lakeid == "AL" | length > 350)
```
