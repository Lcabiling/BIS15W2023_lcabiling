...
#At home, make sure to install tidyverse using install.packages("tidyverse") only once! 
```
...
csv = comma separate value; always use csv
...
```{r}
getwd()
fish <- readr::read_csv("data/Gaeta_etal_CLC_data.csv") #specify a path aka a file within the file 
```
...
glimpse(mammals)
```
structure gives all of the data including the data type, how much variables there, etc. while glimpse shows almost all of the same data in in a neat way, almost like a table. 
