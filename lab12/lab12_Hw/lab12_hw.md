...
```
## [1] "/Users/laurinecabiling/Desktop/BIS15W2023_lcabiling/lab12/lab12_Hw"
```
...
```r
UC_admit <- readr::read_csv(here("lab12", "data", "UC_admit.csv"))
```
...
server <- function(input, output, session) { 
  ...
  output$plot <- renderPlot({
    ...
    ggplot(aes(x=reorder(Ethnicity, FilteredCountFR), y=FilteredCountFR)) + 
      geom_col(color="black", fill="orange", alpha=0.85) +
...
server <- function(input, output, session) { 
  ...
  output$plot <- renderPlot({
    ...
    ggplot(aes(x=Academic_Yr, y=FilteredCountFR)) + 
      geom_col(color="black", fill="lightpink", alpha=0.75) +
...
```r
gabon <- readr::read_csv(here("lab12","data","IvindoData_DryadVersion.csv"))
```
