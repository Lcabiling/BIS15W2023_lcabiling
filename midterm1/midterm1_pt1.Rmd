...
#professor used = `ecosphere %>% count(habitat) %>% arrange(-n)`
```
...
#professor used `ecosphere %>%  select(scientific_name, winter_range_area) %>% slice_max(winter_range_area, n=1)
```

```{r}
slice_min(ecosphere, winter_range_area)

##professor used `ecosphere %>%  select(scientific_name, winter_range_area) %>% slice_min(winter_range_area, n=1)
```
...
#professor used ducks %>% filter(habitat != "Wetland")
```
```{r}
ducks %>% 
...
#the professor did `summarize(mean_log10_mass = mean(log10_mass))
```
...
#need to use `is.na , nothing else will work 
```
