...
Look at what classes should be changed
```{r}
glimpse(mammals)
```
...
view(fish_subset)
```
### instead of btwn, the professor used `radii_lenght_mm >= 2, radii_lenght_mm <= 4`
...
mammals %>% 
  ...
  arrange(newborn)
```
make a new column called newborn_new, and if there is a value of -999.00 change it to NA, and if not then keep newborn value 
```{r}
mammals %>% 
  select(genus, species, newborn) %>%
